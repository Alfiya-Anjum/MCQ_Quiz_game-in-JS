  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    subjectButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            startGame(subject); // Call startGame with the selected subject
        });
    });
});
...
function startGame(selectedSubject) {
    ...
    // Choose the question set based on the selected subject
...
// Ensure you have the startTimer, highlightCurrentPlayer, and endGame functions defined.
...
function reviewAnswers() {
    ...
    currentQuestions.forEach((question, questionIndex) => {
        ...
        // Display each player's choice for this question
        players.forEach(player => {
            const playerAnswer = player.answers[questionIndex]; // Assuming you track players' answers
    ...
    document.addEventListener('DOMContentLoaded', function() {
        document.getElementById('toSubjectSelection').addEventListener('click', function() {
            ...
            // Example: Hide player selection screen, show subject selection screen
    ...
    document.addEventListener('DOMContentLoaded', function() {
        document.getElementById('backToPlayerSelection').addEventListener('click', function() {
            ...
            // Example: Hide subject selection screen, show player selection screen
            document.getElementById('subjectSelectionScreen').style.display = 'none';
    ...
    function loadQuestions(subject) {
        ...
        fetch(subject + '.json') // Assuming your JSON files are named 'HTML.json', 'CSS.json', etc.
            .then(response => response.json())
            .then(data => {
                startQuiz(data);
            })
...
function handleAnswerSelection(selectedAnswer) {
    ...
    answersButtons.forEach(button => {
        ...
        // Assuming each answer in your JSON has 'text' and 'isCorrect' properties
        if (answer === selectedAnswer && selectedAnswer.isCorrect) {
            button.style.backgroundColor = 'green';
        } else if (answer === selectedAnswer && !selectedAnswer.isCorrect) {
            button.style.backgroundColor = 'red';
        } else {
            button.disabled = true; // Optional: Disable other buttons after selection
        }
    });

    console.log("Selected answer:", selectedAnswer);
    // Additional logic for handling correct/incorrect answers can be added here
}
document.addEventListener('DOMContentLoaded', function() {
    const subjectButtons = document.querySelectorAll('.subjectButton');
    subjectButtons.forEach(button => {
    ...
    fetch(url)
        ...
        .then(data => {
            if (data && data.questions) {
                ...
                currentQuestions = data.questions.slice(0, 10); // Select first 10 questions
                startGame(subject);
            } else {
...
function getPlayerColor(index) {
    ...
    // Example: Setting up players (assuming 2 players for simplicity)
players = [
